...
/**
 *
 *      Functional wrapper for the Google Maps v3 library.
 *      Supports marker clustering.
 *
 *      @version:   1.21
 *      @depends:   Google Maps consumer api v3,
 *                  Google Maps MarkerClusterer Api v2
 *
 *      @author:    Michiel Pot.
 *      @license:   Radius College Breda.
 *
 */
	...
	var Gmaps = {
		...
		init : function( container, lat, lon, zoom, scroll )
		{
			this.initial.lat = lat;
			this.initial.lon = lon;
			this.initial.zoom = zoom;

			//  Initiate a new Google Maps object.
			this.map = new google.maps.Map(
				document.getElementById( container ), {

					scrollwheel : scroll,
					zoom: zoom,
					center: this.getCoordinate( lat, lon ),
		...
		doClusterMarkers : function( markers )
		{
		...
		getCoordinate : function( lat, lon )
		{
			//  Return a new GoogleMaps LatLng object ( a valid GPS coordinate ).
		...
		getMarker : function( lat, lon, title )
		{
			//  Return a new GoogleMaps Marker ( a single point on the map ).
			return new google.maps.Marker({

				position: this.getCoordinate( lat, lon ),
				title: title
			});
		},

		getInfoWindow : function( title, body )
		{
			return new google.maps.InfoWindow({
				content:    '<div class="google-maps-infowindow">' +
		...
		getInfoBullet : function( label, value, suffix )
		{
			if ( suffix == undefined )
			{
				suffix  = '';
			...
			//  Format the value to the locale string ( Netherlands )
			//  if it is a valid, flat-divisible number and not an array.
			if ( typeof( value ) !== 'array' && !isNaN( --value ) && value % 1 === 0 )
			{
				value   = value.toLocaleString( 'nl-NL' );
			}

			return '<li>' + label + ': ' + value + ' ' + suffix + '</li>';
		},
