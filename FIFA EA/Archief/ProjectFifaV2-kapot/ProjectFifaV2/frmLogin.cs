    ...
    public partial class frmLogin : Form
    {
        ...
        private void btnRegister_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                using (SqlCommand cmd = new SqlCommand("SELECT COUNT(*) FROM [tbl_users] WHERE Username = @Username", dbh.GetCon()))
                {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("INSERT INTO [tbl_users] ([Username], [Password], [IsAdmin]) VALUES (@Username, @Password, @IsAdmin)"))
                    {
                        cmd.Parameters.AddWithValue("Username", txtUsername.Text);
        ...
        private void btnLogin_Click(object sender, EventArgs e)
        {
            ...
            using (SqlCommand cmd = new SqlCommand("SELECT COUNT(*) FROM [tbl_users] WHERE Username = @Username AND Password = @Password", dbh.GetCon()))
            {
            ...
            {
                bool admin;
                using (SqlCommand cmd = new SqlCommand("SELECT COUNT(*) from [tbl_users] WHERE Username = @Username AND IsAdmin = 1", dbh.GetCon()))
                {
                    cmd.Parameters.AddWithValue("Username", username);
